...
const SearchResultBuyFilters = (props) => {
	...
	useFocusEffect(
		React.useCallback(() => {
			if (props.showSearchResultBuyFilters) {
				if (IOS) {
					setTimeout(() => {
		...
		}, [props.showSearchResultBuyFilters]),
	);
	...
	useEffect(() => {
		if (
			...
			setMiles(
				filterSearchBuyData.distance === null
					? 'Unlimited'
					: filterSearchBuyData.distance.toString(),
			);

			if (filterSearchBuyData.category_id != null) {
				setCategory(filterSearchBuyData.category_id);
			}

			setMin(filterSearchBuyData.min_price);
			setMax(filterSearchBuyData.max_price);
		}
	}, [filterSearchBuyData, filterSearchCategoriesPickerList]);

	useEffect(() => {
		if (filterSearchSubCategoriesPickerList.length > 0) {
			if (filterSearchBuyData.sub_category_id != null) {
	...
	}, [props.showSearchResultBuyFilters, category]);
	...
	const updateSearchHandler = async () => {
		...
		if (
			...
			dispatch(filterSearchItemBuyApi(1, obj, true));
		}
	};
	...
	const resetSearchHandler = () => {
		...
		dispatch(searchItemBuyApi(1, obj, '', true));
	};
	...
	return (
		<Modal
			...
			animationType="fade"
			transparent={true}
			visible={props.showSearchResultBuyFilters}>
